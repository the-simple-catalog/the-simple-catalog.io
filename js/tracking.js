...
class Tracking {
     ...
     * @returns {string} URL routed through CORS proxy
     */
    ...
    static async requestSponsoredProducts(pageId, pageType, context = {}) {
        try {
            ...
            const useProxy = settings.useAdsProxy !== false;
            let endpoint;
            let fetchUrl;
            ...
            if (this.isValidJWT(token)) {
                ...
                if (useProxy) {
                    fetchUrl = this.buildProxiedUrl(`${adsUrl}${endpoint}`);
                    console.log('🔄 [AD SERVING] Using CORS proxy for authenticated request');
                } else {
                    fetchUrl = `${adsUrl}${endpoint}`;
                    console.log('🔄 [AD SERVING] Direct authenticated request (proxy disabled)');
                }
            } else {
